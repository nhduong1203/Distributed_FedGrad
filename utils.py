...
# def get_logging_items(net_list, selected_node_indices, avg_net_prev, avg_net, attackers_idxs, fl_round, filename=None):
    
#     # store the ultimate weight here.
#     logging_list = []
#     recorded_w_list = []
...
#         with open('logging/backdoor_ultimate_w.csv', 'a+') as w_f:
...
def get_logging_items(net_list, selected_node_indices, avg_net_prev, attackers_idxs, fl_round, filename=None):
    ...
    for idx, param in enumerate(avg_net_prev.classifier.parameters()):
            if idx:
                avg_prev_bias = param.data.cpu().numpy()
            else:
                avg_prev_weights = param.data.cpu().numpy()   
