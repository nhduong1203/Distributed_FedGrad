    ...
    def run(self, wandb_ins=None):
        ...
        for flr in range(1, self.fl_round+1):
            ...
            if self.defense_technique == "no-defense":
                ...
                verifier_set = self.select_verifier()
                print("verifier set: ", verifier_set)
                

                pseudo_avg_net = fed_avg_aggregator(net_list, net_freq, device=self.device, model=self.model)
                detect_attacker, detect_honest, honest_nets = [], [], []
                atk_verify = 0
                ...
                for verifier_id in range(len(verifier_set)):
                    ...
                    _, _, pred_g_attacker, pred_g_honest, tpr_fedgrad, fpr_fedgrad, tnr_fedgrad, layer1_inf_time, layer2_inf_t, fedgrad_t = self._defender.exec(client_models=this_net_list,
                                                            num_dps=num_data_points,
                ...
                #-----------------------------------------------------------------------------------------------------------------------------------------------------#
                print(f"Flr: {flr}, attacked verifier: {atk_verify}\n")
                print(f"Flr: {flr}, true positive: {tpr_fedgrad}, true negative: {tnr_fedgrad}, false positive: {fpr_fedgrad}\n")
                print("selected_node_indices: ", selected_node_indices)
                print(f"Flr: {flr}, honest client after all verify: {detect_honest}")
                print(f"Flr: {flr}, detect attacker: {detect_attacker}")
                print(f"Flr: {flr},true attackers: {selected_attackers}")
                not_verified = len(selected_node_indices) - (len(detect_honest) + len(detect_attacker))
            ...
            if(wandb_ins):
                wandb_logging = {'fl_iter': flr, 
                            ...
                            'true_positive_rate': tpr_fedgrad,
                            'true_negative_rate': tnr_fedgrad,
                            'false_positive_rate': fpr_fedgrad,
                            'number_malicious_verifier': atk_verify,
                            'number_not_verified': not_verified,
